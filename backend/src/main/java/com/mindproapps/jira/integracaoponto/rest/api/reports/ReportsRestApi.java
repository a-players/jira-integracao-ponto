...
/* 
    ...
    @Path("/hours/accounts")
    @Produces({MediaType.APPLICATION_JSON})
    public Response getAccountHoursList(@QueryParam("accountId") Integer accountId,
                                        @QueryParam("categoryaccountId") Integer categoryaccountId) {
        if(conditionsHelper.hasUserTempoAdminPermissions()) {
            return Response.ok(reportsService.getAccountHours(accountId, categoryaccountId)).build();
        }
        return Response.status(Response.Status.UNAUTHORIZED).build();
    }
*/
